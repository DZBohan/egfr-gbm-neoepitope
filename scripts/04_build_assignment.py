...
expr={k:{x['sampleId']:float(x['value']) for x in json.loads((ROOT/f'results/expression_audit/{k}.json').read_text())} for k in ['mrna','protein_quantification','gistic','mrna_zscores']}
mutations=json.loads((ROOT/'results/expression_audit/mutations.json').read_text());anymut={r['sampleId'] for r in mutations};picked={r['sampleId'] for r in mutations if r['proteinChange']=='G598V'}
common=set(expr['mrna'])&set(expr['protein_quantification']);amp={s for s,v in expr['gistic'].items() if v>=2}
with (ROOT/'results/expression_audit/sample_table.tsv').open('w',newline='') as out:
...
<header><p class="eyebrow">Immunology course assignment</p><h1>EGFR G598V in Glioblastoma</h1><p>From protein sequence and antigen processing to expression, immunogenicity, and the limits of database validation.</p></header>
...
<p>The selected alteration is EGFR p.Gly598Val (G598V) in glioblastoma. The supplied protein FASTA records identify transcript <code>ENST00000275493</code> and contain 1,210 amino acids each. Direct comparison finds exactly one difference: glycine in the wild type is replaced by valine at residue 598, using one-based numbering of the full precursor protein.</p>
<p class="limitation"><strong>Sequence provenance and VEP limitation.</strong> The sequences below come directly from <code>data/egfr_wt.fa</code> and <code>data/egfr_g598v.fa</code>. No VEP web screenshot, input genomic variant, genome assembly, transcript version, or VEP result export is retained in the repository. Therefore, a completed VEP web annotation cannot be documented here. The protein substitution is verified from the supplied files, not represented as a newly observed VEP result.</p>
''']
...
html.append('<h3>Expression in mutant-bearing versus non-mutated samples</h3><p>The following statistics were recomputed from a fresh cBioPortal API snapshot archived for this audit. The original repository had no expression table. Group membership uses recorded EGFR mutations; “no mutation” means no mutation recorded in this profile, not proof of a completely wild-type tumor.</p>')
rr=[]
...
html.append(source('results/expression_audit/sample_table.tsv'))
...
html.append(f'<p>Recalculated Pearson correlation is {rawr:.3f} for the raw mRNA profile and {zr:.3f} for the archived log2-based mRNA z-score profile. The latter reproduces the displayed 0.92. Agreement across molecular levels supports a coherent association, but does not exclude shared technical or biological confounding.</p>')
rr=[]
...
<p>The DTU service describes self-similarity among its features, but the paper explicitly says the final consensus model does not use the self-similarity feature. The exact deployed wrapper configuration is not archived. A reported similarity column therefore does not demonstrate a fixed self-similarity penalty or explain a particular score causally. HCVKTCPAV has similarity 0.967956; VVMGENNTL is higher at 0.971274. Neither is evidence of measured tolerance. See {cite(dtu,'ICERFIRE service documentation')} and {cite(icer,'the consensus-model description')}.</p>
...
<p>VVMGENNTL occurs in four patients: TCGA-06-0174, TCGA-12-0616, TCGA-19-2620, and TCGA-28-5213. Six distinct TCIA peptide sequences are compatible with the local G598V sequence, including GPHCVKTCPAV, which must be included when enumerating this site. These entries span seven patients in the filtered EGFR table. The export has no genomic variant column or HLA assignment, so sequence compatibility alone does not verify each patient's mutation or HLA restriction and is not a population prevalence estimate.</p>
